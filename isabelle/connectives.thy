...
(*We illustrate how the logical connectives could have been defined in terms of equality resp. disequality.
 (We actually work with them as they are provided by Isabelle/HOL (with some notational changes).*)
...
(*Moreover, they are also definable using indefinite descriptions \<epsilon> resp. \<delta> and the \<^bold>\<Pi>\<^sub>1\<^sub>1\<^sub>1/\<^bold>O combinator*)
...
(*Henkin (1963) also defines \<iota>::(o\<Rightarrow>o)\<Rightarrow>o via equality, namely as: \<Q> \<^bold>I.
Note, however, that in Isabelle/HOL the term \<iota>::(o\<Rightarrow>o)\<Rightarrow>o is not introduced as a definition.
Instead, \<iota>::(o\<Rightarrow>o)\<Rightarrow>o is an instance of \<iota>::('a\<Rightarrow>o)\<Rightarrow>'a, which is an axiomatized (polymorphic) constant.*)
lemma "\<iota> = \<Q> \<^bold>I" nitpick oops (*countermodel*)
