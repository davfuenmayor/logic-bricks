...
definition leftRange::"Rel('a,'b) \<Rightarrow> Set('a)"
  where "leftRange  \<equiv> \<^bold>A \<ggreater>\<^sub>2 \<exists>"
definition rightRange::"Rel('a,'b) \<Rightarrow> Set('b)"
  where "rightRange \<equiv> \<^bold>C \<ggreater>\<^sub>2 \<exists>"
...
definition leftDualRange::"Rel('a,'b) \<Rightarrow> Set('a)"
  where "leftDualRange \<equiv>  \<^bold>A \<ggreater>\<^sub>2 \<forall>"
definition rightDualRange::"Rel('a,'b) \<Rightarrow> Set('b)"
  where "rightDualRange \<equiv> \<^bold>C \<ggreater>\<^sub>2 \<forall>"
...
(* TODO: deterministic/unique/univalent- left- resp right-range *)
...
definition rightUnique::"Set(Rel('a,'b))" 
  where "rightUnique \<equiv> deterministic \<ggreater> \<forall>"
definition leftUnique::"Set(Rel('a,'b))"
  where "leftUnique \<equiv>  \<^bold>C \<ggreater> deterministic \<ggreater> \<forall>"
...
definition rightTotal::"Set(Rel('a,'b))"
  where "rightTotal \<equiv> rightRange \<ggreater> \<forall>"
definition leftTotal::"Set(Rel('a,'b))"
  where "leftTotal \<equiv> leftRange \<ggreater> \<forall>"
...
definition totalFunction::"Set(Rel('a,'b))"
  where "totalFunction \<equiv> totalDeterministic \<ggreater> \<forall>"
...
lemma "asRel f = f \<ggreater> \<Q>" unfolding rel_defs ..
...
lemma asRel_def2: "asRel = inverse \<ggreater> \<^bold>C" unfolding rel_defs func_defs comb_defs ..
...
definition intoRel::"Set('a \<Rightarrow> 'b) \<Rightarrow> Rel('a,'b)" ("intoRel")
  where "intoRel \<equiv> \<^bold>C (\<^bold>T \<ggreater> image)"
...
lemma "asFun R = R \<ggreater> \<epsilon>" unfolding rel_defs ..
...
lemma asFun_def2: "totalFunction R \<Longrightarrow> asFun R = R \<ggreater> \<iota>" \<comment> \<open>alternative definition for total functions\<close>
...
(* TODO: define in terms of (dual)interpolants? *)
text \<open>In analogy to functions, relations can also be composed, as follows:\<close>
definition relComp::"Rel('a,'b) \<Rightarrow> Rel('b,'c) \<Rightarrow>  Rel('a,'c)" (infixr ";" 55)
  where "(;) =  \<^bold>B\<^sub>2\<^sub>2 (\<sqinter>) \<^bold>A \<smile>"
text \<open>Again, we can in fact define an operator that acts as a "dual" to relation-composition:\<close>
definition relDualComp::"Rel('c,'a) \<Rightarrow> Rel('a,'b) \<Rightarrow> Rel('c,'b)" (infixr "\<dagger>" 55)
  where "(\<dagger>) \<equiv> \<^bold>B\<^sub>2\<^sub>2 (\<squnion>) \<^bold>A \<smile>"

declare relDualComp_def[rel_defs] relComp_def[rel_defs]

lemma "R\<^sub>1 ; R\<^sub>2 = (\<lambda>a b. R\<^sub>1 a \<sqinter> R\<^sub>2\<^sup>\<smile> b)" unfolding rel_defs comb_defs ..
lemma "R\<^sub>1 \<dagger> R\<^sub>2 = (\<lambda>a b. R\<^sub>1 a \<squnion> R\<^sub>2\<^sup>\<smile> b)" unfolding rel_defs comb_defs ..
lemma "R\<^sub>1 ; R\<^sub>2 = (\<lambda>a b. \<exists>c. R\<^sub>1 a c \<and> R\<^sub>2 c b)" unfolding rel_defs func_defs comb_defs ..
lemma "R\<^sub>1 \<dagger> R\<^sub>2 = (\<lambda>a b. \<forall>c. R\<^sub>1 a c \<or> R\<^sub>2 c b)" unfolding rel_defs func_defs comb_defs ..
lemma "R\<^sub>1 ; R\<^sub>2 = (\<lambda>a b. \<exists>(interpolants R\<^sub>1 R\<^sub>2 a b))" unfolding rel_defs func_defs comb_defs ..
lemma "R\<^sub>1 \<dagger> R\<^sub>2 = (\<lambda>a b. \<forall>(dualInterpolants R\<^sub>1 R\<^sub>2 a b))" unfolding rel_defs func_defs comb_defs ..
lemma "R\<^sub>1 ; R\<^sub>2 = interpolants R\<^sub>1 R\<^sub>2 \<ggreater>\<^sub>2 \<exists>" unfolding rel_defs func_defs comb_defs ..
lemma "R\<^sub>1 \<dagger> R\<^sub>2 = dualInterpolants R\<^sub>1 R\<^sub>2 \<ggreater>\<^sub>2 \<forall>" unfolding rel_defs func_defs comb_defs ..
lemma relComp_def2:     "(;) = interpolants \<ggreater>\<^sub>4 \<exists>" unfolding rel_defs func_defs comb_defs ..
lemma relDualComp_def2: "(\<dagger>) = dualInterpolants \<ggreater>\<^sub>4 \<forall>" unfolding rel_defs func_defs comb_defs ..

text \<open>Unsurprisingly, (relational) composition and dual-composition are dual wrt. (relational) complement.\<close>
lemma relCompDuality1: "R \<dagger> T = ((R\<^sup>\<midarrow>) ; (T\<^sup>\<midarrow>))\<^sup>\<midarrow>" 
  unfolding rel_defs func_defs comb_defs by auto
lemma relCompDuality2: "R ; T = ((R\<^sup>\<midarrow>) \<dagger> (T\<^sup>\<midarrow>))\<^sup>\<midarrow>" 
  unfolding rel_defs func_defs comb_defs by auto

text \<open>Moreover, relation (dual)composition and (dis)equality satisfy the monoid conditions\<close>
lemma relComp_assoc: "(R ; T) ; V = R ; (T ; V)" \<comment> \<open>associativity\<close>
  unfolding rel_defs func_defs comb_defs by auto                   
lemma relComp_id1: "\<Q> ; R = R"                     \<comment> \<open>identity 1\<close>
  unfolding rel_defs func_defs comb_defs by simp                   
lemma relComp_id2: "R ; \<Q> = R"                     \<comment> \<open>identity 2\<close>
  unfolding rel_defs func_defs comb_defs by simp   
lemma relCompDual_assoc: "(R \<dagger> T) \<dagger> V = R \<dagger> (T \<dagger> V)" \<comment> \<open>associativity\<close>
  unfolding rel_defs func_defs comb_defs by auto                   
lemma relCompDual_id1: "\<D> \<dagger> R = R"                     \<comment> \<open>identity 1\<close>
  unfolding rel_defs func_defs comb_defs by simp                   
lemma relCompDual_id2: "R \<dagger> \<D> = R"                     \<comment> \<open>identity 2\<close>
  unfolding rel_defs func_defs comb_defs by auto 

text \<open>Transpose acts as an "antihomomorphism" wrt. composition as well as its dual.\<close>
lemma relComp_antihom:     "(R ; T)\<^sup>\<smile> = ((T\<^sup>\<smile>) ; (R\<^sup>\<smile>))" unfolding rel_defs func_defs comb_defs by auto
lemma relCompDual_antihom: "(R \<dagger> T)\<^sup>\<smile> = ((T\<^sup>\<smile>) \<dagger> (R\<^sup>\<smile>))" unfolding rel_defs func_defs comb_defs by auto

text \<open>In a similar spirit, we have:\<close>
lemma "(R ; T)\<^sup>\<sim> = ((T\<^sup>\<sim>) \<dagger> (R\<^sup>\<sim>))" unfolding rel_defs func_defs comb_defs by auto
lemma "(R \<dagger> T)\<^sup>\<sim> = ((T\<^sup>\<sim>) ; (R\<^sup>\<sim>))" unfolding rel_defs func_defs comb_defs by auto


subsubsection \<open>Residuals\<close>

text \<open>Introduce residuals (on the left resp. right) wrt. composition taken as \<open>(;)\<close>.\<close>
definition residualOnRight::"Rel('c,'a) \<Rightarrow> Rel('c,'b) \<Rightarrow> Rel('a,'b)" (infix "\<Zrres>" 99)
  where "R \<Zrres> S \<equiv> (R\<^sup>\<sim>) \<dagger> S"
definition residualOnLeft::"Rel('a,'c) \<Rightarrow> Rel('b,'c) \<Rightarrow> Rel('a,'b)" (infix "\<Zndres>" 99)
  where "R \<Zndres> S \<equiv>  R  \<dagger> (S\<^sup>\<sim>)"
...
lemma residualOnRight_def2: "R \<Zrres> S = ((R\<^sup>\<smile>) ; (S\<^sup>\<midarrow>))\<^sup>\<midarrow>" unfolding rel_defs func_defs comb_defs by simp
...
abbreviation(input) residualOnRight_t (infix "\<Zdres>" 99)
  where "R \<Zdres> S \<equiv> S \<Zrres> R"
abbreviation(input) residualOnLeft_t (infix "\<Znrres>" 99)
  where "R \<Znrres> S \<equiv> S \<Zndres> R"
...
definition relKernel::"Rel('a,'b) \<Rightarrow> ERel('a)"
  where "relKernel \<equiv> \<^bold>\<Psi>\<^sub>2\<^sub>1 (\<sqinter>)"
...
lemma "relPullback = \<^bold>C relPullback \<ggreater>\<^sub>2 \<^bold>C" apply (subst relPullback_symm) unfolding comb_defs ..
...
lemma "relEqualizer = relPullback \<ggreater>\<^sub>2 \<^bold>W" unfolding rel_defs comb_defs ..

text \<open>Note that we can swap the roles of "points" and "functions" in the above definitions using permutators.\<close>
lemma "\<^bold>R\<^sub>3 relEqualizer x = (\<lambda>R T. R x \<sqinter> T x)" unfolding rel_defs comb_defs ..
lemma "\<^bold>C\<^sub>3\<^sub>4\<^sub>1\<^sub>2 relPullback x y = (\<lambda>R T. R x \<sqinter> T y)" unfolding rel_defs comb_defs ..
...
lemma "relCoequalizer = relPushout \<ggreater>\<^sub>2 \<^bold>W" unfolding rel_defs comb_defs ..
...
lemma rightImage_def2: "rightImage = image \<ggreater>\<^sub>2 \<Union>"
...
lemma "rightImage     (asRel f) = image f"     unfolding rel_defs comb_defs func_defs by auto
lemma "rightDualImage (asRel f) = dualimage f" unfolding rel_defs comb_defs func_defs by auto
lemma "leftImage      (asRel f) = preimage f"  unfolding rel_defs comb_defs func_defs by auto
lemma "leftDualImage  (asRel f) = preimage f"  unfolding rel_defs comb_defs func_defs by auto

lemma "totalFunction R \<Longrightarrow> image     (asFun R) = rightImage R" unfolding rel_defs comb_defs func_defs by (metis someI)
lemma "totalFunction R \<Longrightarrow> dualimage (asFun R) = rightDualImage R" unfolding rel_defs comb_defs func_defs by (metis someI)
lemma "totalFunction R \<Longrightarrow> preimage (asFun R)  = leftImage R" unfolding rel_defs comb_defs func_defs by (metis someI)
lemma "totalFunction R \<Longrightarrow> preimage (asFun R)  = leftDualImage R" unfolding rel_defs comb_defs func_defs by (metis someI)
...
definition rightCodualBound::"Rel('a,'b) \<Rightarrow> SetOp('a,'b)"
  where  "rightCodualBound \<equiv> \<^bold>C (\<^bold>B\<^sub>2\<^sub>0 (\<squnion>) \<^bold>C)"              (*cf. rightImage (using \<squnion>)*)
definition leftCodualBound::"Rel('a,'b) \<Rightarrow> SetOp('b,'a)"
  where  "leftCodualBound \<equiv> \<^bold>C (\<^bold>B\<^sub>2\<^sub>0 (\<squnion>) \<^bold>A)"                (*cf. leftImage (using \<squnion>)*)

declare rightBound_def[rel_defs] leftBound_def[rel_defs] rightCodualBound_def[rel_defs] leftCodualBound_def[rel_defs]

notation(input) rightBound ("_-rightBound") and  leftBound ("_-leftBound") and
                rightCodualBound ("_-rightCodualBound") and  leftCodualBound ("_-leftCodualBound")
...
lemma "R-rightCodualBound A = (\<lambda>b. R\<^sup>\<smile> b \<squnion> A)" unfolding rel_defs comb_defs ..
lemma "R-leftCodualBound  B = (\<lambda>a. R a  \<squnion> B)" unfolding rel_defs comb_defs ..
...
lemma "R-rightCodualBound A = (\<lambda>b. \<forall>a. \<not>A a \<rightarrow> R a b)" unfolding rel_defs func_defs comb_defs by auto
lemma "R-leftCodualBound  B = (\<lambda>a. \<forall>b. \<not>B b \<rightarrow> R a b)" unfolding rel_defs func_defs comb_defs by auto
...
text \<open>Convenient characterizations in terms of big-union and big-intersection.\<close>
lemma rightBound_def2: "rightBound = image \<ggreater>\<^sub>2 \<Inter>"
  unfolding rel_defs func_defs comb_defs by fastforce
...
lemma rightBoundCodual_defT: "R-rightCodualBound = R\<^sup>\<smile>-leftCodualBound" unfolding rel_defs comb_defs ..
lemma leftBoundCodual_defT: "R-leftCodualBound = R\<^sup>\<smile>-rightCodualBound" unfolding rel_defs comb_defs ..
...
definition relativeBorder :: "Rel('a,'b) \<Rightarrow> Set('a) \<Rightarrow> Set('b) \<Rightarrow> Set('a)"
  where "relativeBorder \<equiv> leftImage \<ggreater> (\<^bold>B\<^sub>1\<^sub>1 (\<inter>) \<^bold>I)"
...
abbreviation(input) relativeCoborder :: "Rel('a,'b) \<Rightarrow> Set('a) \<Rightarrow> Set('b) \<Rightarrow> Set('b)"
  where "relativeCoborder \<equiv> \<smile> \<ggreater> relativeBorder \<ggreater> \<^bold>C"
...
lemma relativeCoborder_def2: "relativeCoborder = rightImage \<ggreater>\<^sub>2 (\<inter>)" unfolding relativeBorder_def rel_defs func_defs comb_defs by auto
...
definition relativeInterior :: "Rel('a,'b) \<Rightarrow> Set('a) \<Rightarrow> Set('b) \<Rightarrow> Set('a)"
  where "relativeInterior \<equiv> leftImage \<ggreater> (\<^bold>B\<^sub>1\<^sub>1 (\<setminus>) \<^bold>I)"
...
abbreviation(input) relativeExterior :: "Rel('a,'b) \<Rightarrow> Set('a) \<Rightarrow> Set('b) \<Rightarrow> Set('b)"
  where "relativeExterior \<equiv> \<smile> \<ggreater> relativeInterior \<ggreater> \<^bold>C"
...
definition relLiftEE :: "Rel('a,'b) \<Rightarrow> Rel(Set('a),Set('b))"
  where "relLiftEE \<equiv> relativeBorder \<ggreater>\<^sub>3 \<exists>"

declare relLiftEE_def[rel_defs]

lemma relLiftEE_def2: "relLiftEE = relativeCoborder \<ggreater>\<^sub>3 \<exists>" unfolding rel_defs func_defs comb_defs by metis
...
definition relLiftAE_left :: "Rel('a,'b) \<Rightarrow> Rel(Set('a),Set('b))"
  where "relLiftAE_left  \<equiv> relativeInterior \<ggreater>\<^sub>3 \<nexists>"
definition relLiftAE_right :: "Rel('a,'b) \<Rightarrow> Rel(Set('a),Set('b))"
  where "relLiftAE_right \<equiv> relativeExterior \<ggreater>\<^sub>3 \<nexists>"
...
lemma rightImage_def3: "rightImage = relLiftAE_right \<ggreater>\<^sub>2 \<Union>"  unfolding rel_defs func_defs comb_defs by metis
lemma rightBound_def3: "rightBound = relLiftAA \<ggreater>\<^sub>2 \<Union>" unfolding rel_defs func_defs comb_defs apply (rule ext)+ sorry (*TODO: reconstruct in kernel*) 
lemma leftImage_def3:  "leftImage  = \<smile> \<ggreater> relLiftAE_right \<ggreater>\<^sub>2 \<Union>"  unfolding rel_defs func_defs comb_defs apply (rule ext) by fastforce
lemma leftBound_def3:  "leftBound  = \<smile> \<ggreater> relLiftAA \<ggreater>\<^sub>2 \<Union>"  unfolding rel_defs func_defs comb_defs apply (rule ext)+ sorry (*TODO: reconstruct in kernel*)


subsection \<open>Extension to higher-arities\<close>
...
definition eq3::"ERel\<^sub>3('a)" ("\<Q>\<^sub>3")
  where"\<Q>\<^sub>3 \<equiv> (\<lambda>x y z. x = y \<and> y = z)"
definition diseq3::"ERel\<^sub>3('a)" ("\<D>\<^sub>3")
  where"\<D>\<^sub>3 \<equiv> (\<lambda>x y z. x \<noteq> y \<or> y \<noteq> z)"

declare eq3_def[rel_defs] diseq3_def[rel_defs]

lemma eq3_dual: "\<Q>\<^sub>3 a b c = (\<not>(\<D>\<^sub>3 a b c))" unfolding rel_defs by simp

text \<open>Lifting (cartesian) product and (disjoint) sum to higher arities.\<close>
definition product3::"Set('a) \<Rightarrow> Set('b) \<Rightarrow> Set('c) \<Rightarrow> Rel\<^sub>3('a,'b,'c)" ("_ \<times> _ \<times> _")
  where "A \<times> B \<times> C \<equiv> \<lambda>a b c. A a \<and> B b \<and> C c"
definition sum3::"Set('a) \<Rightarrow> Set('b) \<Rightarrow> Set('c) \<Rightarrow> Rel\<^sub>3('a,'b,'c)" ("_ \<uplus> _ \<uplus> _")
  where "A \<uplus> B \<uplus> C \<equiv> \<lambda>a b c. A a \<or> B b \<or> C c"
\<comment> \<open>and so on ...\<close>

declare product3_def[rel_defs] sum3_def[rel_defs]
...
lemma "image f A = (\<lambda>b. \<exists>a. f a = b \<and> A a)" unfolding rel_defs func_defs comb_defs ..
lemma "image\<^sub>2 f A B = (\<lambda>c. \<exists>a b. f a b = c \<and> A a \<and> B b)" unfolding rel_defs func_defs comb_defs ..
lemma "image\<^sub>3 f A B C = (\<lambda>d. \<exists>a b c. f a b c = d \<and> A a \<and> B b \<and> C c)" unfolding rel_defs func_defs comb_defs ..
\<comment> \<open>...\<open>image\<^sub>n f A\<^sub>1 ...A\<^sub>n = (\<lambda>x. \<exists>a\<^sub>1 ... a\<^sub>n. f a\<^sub>1 ... a\<^sub>n = x \<and> A\<^sub>1 a\<^sub>1 \<and> ... A\<^sub>n a\<^sub>n\<close>\<close>
...
definition preimage2 :: "('a \<Rightarrow> 'b \<Rightarrow> 'c) \<Rightarrow> Set('c) \<Rightarrow> Rel('a,'b)" ("preimage\<^sub>2")
  where "preimage\<^sub>2 f C \<equiv> f \<ggreater>\<^sub>2 C"
definition preimage3 :: "('a \<Rightarrow> 'b \<Rightarrow> 'c \<Rightarrow> 'd) \<Rightarrow> Set('d) \<Rightarrow> Rel\<^sub>3('a,'b,'c)" ("preimage\<^sub>3")
  where "preimage\<^sub>3 f D \<equiv> f \<ggreater>\<^sub>3 D"
\<comment> \<open>... \<open>preimage\<^sub>n f X \<equiv> f \<ggreater>\<^sub>n X\<close>\<close>

declare preimage2_def[func_defs] preimage3_def[func_defs]

text \<open>Similarly, we lift the notion of "dualimage" towards higher arities, recalling that:\<close>
...
declare dualimage2_def[func_defs] dualimage3_def[func_defs]

lemma "dualimage f A = (\<lambda>b. \<forall>a. f a = b \<rightarrow> A a)" unfolding rel_defs func_defs comb_defs ..
lemma "dualimage\<^sub>2 f A B = (\<lambda>c. \<forall>a b. f a b = c \<rightarrow> A a \<or> B b)" unfolding rel_defs func_defs comb_defs ..
lemma "dualimage\<^sub>3 f A B C = (\<lambda>d. \<forall>a b c. f a b c = d \<rightarrow> A a \<or> B b \<or> C c)" unfolding rel_defs func_defs comb_defs ..
\<comment> \<open>...\<open>dualimage\<^sub>n f A\<^sub>1 ...A\<^sub>n = (\<lambda>x. \<forall>a\<^sub>1 ... a\<^sub>n. f a\<^sub>1 ... a\<^sub>n = x \<rightarrow> A\<^sub>1 a\<^sub>1 \<or> ... A\<^sub>n a\<^sub>n)\<close>\<close>

lemma image2_dual1: "image\<^sub>2 f A B   = \<midarrow>((dualimage\<^sub>2 f) (\<midarrow>A) (\<midarrow>B))"  unfolding rel_defs func_defs comb_defs by simp
lemma image3_dual1: "image\<^sub>3 f A B C = \<midarrow>((dualimage\<^sub>3 f) (\<midarrow>A) (\<midarrow>B) (\<midarrow>C))" unfolding rel_defs func_defs comb_defs by auto
lemma image2_dual2: "dualimage\<^sub>2 f A B   = \<midarrow>((image\<^sub>2 f) (\<midarrow>A) (\<midarrow>B))"  unfolding rel_defs func_defs comb_defs by simp
lemma image3_dual2: "dualimage\<^sub>3 f A B C = \<midarrow>((image\<^sub>3 f) (\<midarrow>A) (\<midarrow>B) (\<midarrow>C))" unfolding rel_defs func_defs comb_defs by simp
\<comment> \<open>and so on ...\<close>
...
lemma "rightImage = image \<ggreater>\<^sub>2 \<Union>" unfolding rightImage_def2 ..
...
definition rightImage2 ::"Rel\<^sub>3('a,'b,'c) \<Rightarrow> Set('a) \<Rightarrow> Set('b) \<Rightarrow> Set('c)"  ("rightImage\<^sub>2")
  where "rightImage\<^sub>2 \<equiv> image\<^sub>2 \<ggreater>\<^sub>3 \<Union>"
definition rightImage3 ::"Rel\<^sub>4('a,'b,'c,'d) \<Rightarrow> Set('a) \<Rightarrow> Set('b) \<Rightarrow> Set('c) \<Rightarrow> Set('d)"  ("rightImage\<^sub>3")
  where "rightImage\<^sub>3 \<equiv> image\<^sub>3 \<ggreater>\<^sub>4 \<Union>"
\<comment> \<open>... \<open>rightImage\<^sub>n \<equiv> image\<^sub>n \<ggreater>\<^sub>n\<^sub>+\<^sub>1 \<Union>\<close>\<close>
...
lemma rightImage2_def2: "rightImage\<^sub>2 R A B = (\<lambda>c. \<^bold>R\<^sub>3 R c \<sqinter>\<^sup>2 (A \<times> B))" unfolding rel_defs func_defs comb_defs by fast
lemma rightImage2_def3: "rightImage\<^sub>3 R A B C = (\<lambda>d. \<^bold>R\<^sub>4 R d \<sqinter>\<^sup>3 (A \<times> B \<times> C))" unfolding rel_defs func_defs comb_defs by fast
\<comment>                 \<open>... \<open>rightImage\<^sub>n R A\<^sub>1 ...A\<^sub>n = (\<lambda>x. \<^bold>R\<^sub>n\<^sub>+\<^sub>1 R x \<sqinter>\<^sup>n (A\<^sub>1 \<times> \<dots> \<times> A\<^sub>n))\<close>\<close>

text \<open>Now, recall that for binary relations we also have that:\<close>
lemma "rightBound = image \<ggreater>\<^sub>2 \<Inter>" unfolding rightBound_def2 ..
...
definition rightBound2 ::"Rel\<^sub>3('a,'b,'c) \<Rightarrow> Set('a) \<Rightarrow> Set('b) \<Rightarrow> Set('c)"  ("rightBound\<^sub>2")
  where "rightBound\<^sub>2 \<equiv> image\<^sub>2 \<ggreater>\<^sub>3 \<Inter>"
definition rightBound3 ::"Rel\<^sub>4('a,'b,'c,'d) \<Rightarrow> Set('a) \<Rightarrow> Set('b) \<Rightarrow> Set('c) \<Rightarrow> Set('d)"  ("rightBound\<^sub>3")
  where "rightBound\<^sub>3 \<equiv> image\<^sub>3 \<ggreater>\<^sub>4 \<Inter>"
\<comment> \<open>... \<open>rightBound\<^sub>n \<equiv> image\<^sub>n \<ggreater>\<^sub>n\<^sub>+\<^sub>1 \<Inter>\<close>\<close>

declare rightBound2_def[rel_defs] rightBound3_def[rel_defs]

text \<open>And, analogously as before, we can check:\<close>
lemma rightBound2_def2: "rightBound\<^sub>2 R A B = (\<lambda>c. A \<times> B \<subseteq>\<^sup>2 \<^bold>R\<^sub>3 R c)" unfolding rel_defs func_defs comb_defs by fast
lemma rightBound2_def3: "rightBound\<^sub>3 R A B C = (\<lambda>d. A \<times> B \<times> C \<subseteq>\<^sup>3 \<^bold>R\<^sub>4 R d)" unfolding rel_defs func_defs comb_defs by fast
\<comment>                 \<open>... \<open>rightBound\<^sub>n R A\<^sub>1 ...A\<^sub>n = (\<lambda>x. A\<^sub>1 \<times> \<dots> \<times> A\<^sub>n \<subseteq>\<^sup>n \<^bold>R\<^sub>n\<^sub>+\<^sub>1 R x)\<close>\<close>
...
definition rightDualImage2::"Rel\<^sub>3('a,'b,'c) \<Rightarrow> Set('a) \<Rightarrow> Set('b) \<Rightarrow> Set('c)"  ("rightDualImage\<^sub>2")
  where "rightDualImage\<^sub>2 R \<equiv> \<lambda>A B. \<lambda>c. \<^bold>R\<^sub>3 R c \<subseteq>\<^sup>2 (A \<uplus> B)"
definition rightDualImage3::"Rel\<^sub>4('a,'b,'c,'d) \<Rightarrow> Set('a) \<Rightarrow> Set('b) \<Rightarrow> Set('c) \<Rightarrow> Set('d)"  ("rightDualImage\<^sub>3")
  where "rightDualImage\<^sub>3 R \<equiv> \<lambda>A B C. \<lambda>d. \<^bold>R\<^sub>4 R d \<subseteq>\<^sup>3 (A \<uplus> B \<uplus> C)"
\<comment> \<open>... \<open>rightDualImage\<^sub>n R \<equiv> (\<lambda>A\<^sub>1 ...A\<^sub>n. \<lambda>x. \<^bold>R\<^sub>n\<^sub>+\<^sub>1 R x \<subseteq>\<^sup>n (A\<^sub>1 \<times> \<dots> \<times> A\<^sub>n))\<close>\<close>

declare rightDualImage2_def[rel_defs] rightDualImage3_def[rel_defs]

lemma "rightDualImage\<^sub>2 R A B = (\<lambda>c. \<forall>a b. R a b c \<rightarrow> A a \<or> B b)" unfolding rel_defs func_defs comb_defs by simp
lemma "rightDualImage\<^sub>3 R A B C = (\<lambda>d. \<forall>a b c. R a b c d \<rightarrow> A a \<or> B b \<or> C c)" unfolding rel_defs func_defs comb_defs by simp
\<comment> \<open>...\<open>rightDualImage\<^sub>n R A\<^sub>1 ...A\<^sub>n = (\<lambda>x. \<forall>a\<^sub>1 ... a\<^sub>n. R a\<^sub>1 ... a\<^sub>n x \<rightarrow> A\<^sub>1 a\<^sub>1 \<or> ... A\<^sub>n a\<^sub>n)\<close>\<close>

text \<open>Check dualities:\<close>
lemma rightImage2_dual1: "rightImage\<^sub>2 R A B = \<midarrow>(rightDualImage\<^sub>2 R (\<midarrow>A) (\<midarrow>B))" unfolding rel_defs func_defs comb_defs by fast
lemma rightImage3_dual1: "rightImage\<^sub>3 R A B C = \<midarrow>(rightDualImage\<^sub>3 R (\<midarrow>A) (\<midarrow>B) (\<midarrow>C))" unfolding rel_defs func_defs comb_defs by fast
lemma rightImage2_dual2: "rightDualImage\<^sub>2 R A B = \<midarrow>(rightImage\<^sub>2 R (\<midarrow>A) (\<midarrow>B))" unfolding rel_defs func_defs comb_defs by fast
lemma rightImage3_dual2: "rightDualImage\<^sub>3 R A B C = \<midarrow>(rightImage\<^sub>3 R (\<midarrow>A) (\<midarrow>B) (\<midarrow>C))" unfolding rel_defs func_defs comb_defs by fast
...
definition rightCodualBound2::"Rel\<^sub>3('a,'b,'c) \<Rightarrow> Set('a) \<Rightarrow> Set('b) \<Rightarrow> Set('c)" ("rightCodualBound\<^sub>2")
  where "rightCodualBound\<^sub>2 R  \<equiv> \<lambda>A B. \<lambda>c. \<^bold>R\<^sub>3 R c \<squnion>\<^sup>2 (A \<uplus> B)"
definition rightCodualBound3::"Rel\<^sub>4('a,'b,'c,'d) \<Rightarrow> Set('a) \<Rightarrow> Set('b) \<Rightarrow> Set('c) \<Rightarrow> Set('d)"  ("rightCodualBound\<^sub>3")
  where "rightCodualBound\<^sub>3 R  \<equiv> \<lambda>A B C. \<lambda>d. \<^bold>R\<^sub>4 R d \<squnion>\<^sup>3 (A \<uplus> B \<uplus> C)"
\<comment> \<open>... \<open>rightCodualBound\<^sub>n R \<equiv> (\<lambda>A\<^sub>1 ...A\<^sub>n. \<lambda>x. \<^bold>R\<^sub>n\<^sub>+\<^sub>1 R x \<squnion>\<^sup>n (A\<^sub>1 \<uplus> \<dots> \<uplus> A\<^sub>n))\<close>\<close>

declare rightCodualBound2_def[rel_defs] rightCodualBound3_def[rel_defs]

lemma "rightCodualBound\<^sub>2 R A B = (\<lambda>c. \<forall>a b. \<not>(A a \<or> B b) \<rightarrow> R a b c)" unfolding rel_defs func_defs comb_defs by fast
lemma "rightCodualBound\<^sub>3 R A B C = (\<lambda>d. \<forall>a b c. \<not>(A a \<or> B b \<or> C c) \<rightarrow> R a b c d)" unfolding rel_defs func_defs comb_defs by fast
\<comment> \<open>...\<open>rightCodualImage\<^sub>n R A\<^sub>1 ...A\<^sub>n = (\<lambda>x. \<forall>a\<^sub>1 ... a\<^sub>n. \<not>(A\<^sub>1 a\<^sub>1 \<or> ... A\<^sub>n a\<^sub>n) \<rightarrow> R a\<^sub>1 ... a\<^sub>n x)\<close>\<close>
...
text \<open>Check dualities:\<close>
lemma rightBound2_dual1: "rightBound\<^sub>2 R A B = \<midarrow>(rightDualBound\<^sub>2 R (\<midarrow>A) (\<midarrow>B))" unfolding rel_defs func_defs comb_defs by fast
lemma rightBound3_dual1: "rightBound\<^sub>3 R A B C = \<midarrow>(rightDualBound\<^sub>3 R (\<midarrow>A) (\<midarrow>B) (\<midarrow>C))" unfolding rel_defs func_defs comb_defs by fast
lemma rightBound2_dual2: "rightDualBound\<^sub>2 R A B = \<midarrow>(rightBound\<^sub>2 R (\<midarrow>A) (\<midarrow>B))" unfolding rel_defs func_defs comb_defs by fast
lemma rightBound3_dual2: "rightDualBound\<^sub>3 R A B C = \<midarrow>(rightBound\<^sub>3 R (\<midarrow>A) (\<midarrow>B) (\<midarrow>C))" unfolding rel_defs func_defs comb_defs by fast
