...
no_notation
  ...
  set.apr  (infixl "*>" 54) and set.bind  (infixl "\<bind>" 54) and set.mcomp' (infixr "\<Zfinj>" 56) and set.acomp' (infixr "\<Zinj>" 56)
notation
  ...
  set.apr  (infixl "*>\<^sup>s" 54) and set.bind  (infixl "\<bind>\<^sup>s" 54) and set.mcomp' (infixr "\<Zfinj>\<^sup>s" 56) and set.acomp' (infixr "\<Zinj>\<^sup>s" 56)
...
abbreviation(input) fmap3::"('a \<Rightarrow> 'b \<Rightarrow> 'c \<Rightarrow> 'd) \<Rightarrow> (Rel('e,'a) \<Rightarrow> Rel('e,'b) \<Rightarrow> Rel('e,'c) \<Rightarrow> Rel('e,'d))"
  where "fmap3 \<equiv> set.fmap3 \<ggreater> env.fmap3"
\<comment> \<open>...and so on\<close>

lemma "fmap0 f A = {f}" unfolding all_defs ..
lemma "fmap1 f A = (\<lambda>x y. \<exists>a. f a = y \<and> A x a)" unfolding all_defs ..
lemma "fmap2 f A B = (\<lambda>y z. \<exists>a b. f a b = z \<and> A y a \<and> B y b)" unfolding all_defs ..
\<comment> \<open>...and so on\<close>
...
abbreviation(input) join::"Rel('c,Rel('c,'a)) \<Rightarrow> Rel('c,'a)"
  where "join \<equiv> env.fmap (set.intoArrowM \<ggreater>\<^sub>2 set.join) \<ggreater> env.join"
...
abbreviation(input) asArrowM::"('a \<Rightarrow> 'c) \<Rightarrow> ('a \<Rightarrow> Rel('b,'c))"
  where "asArrowM \<equiv> set.asArrowM \<ggreater> env.asArrowM"
...
abbreviation(input) intoArrowM::"Rel('b,'a \<Rightarrow> 'c) \<Rightarrow> ('a \<Rightarrow> Rel('b,'c))"
  where "intoArrowM \<equiv> (env.fmap \<ggreater>\<^sub>2 env.intoArrowM) set.intoArrowM"
...
abbreviation(input) intoArrowA::"('a \<Rightarrow> Rel('b,'c)) \<Rightarrow> Rel('b,'a \<Rightarrow> 'c)"
  where "intoArrowA \<equiv> env.intoArrowA \<ggreater>\<^sub>2 set.intoArrowA"
...
text \<open>In fact, Kleisli composition can be used to model composition of ternary relations.\<close>
lemma "R\<^sub>1 \<Zfinj> R\<^sub>2 = (\<lambda>a b c. \<exists>x. R\<^sub>1 a b x \<and> R\<^sub>2 x b c)" unfolding all_defs by auto
